...
export function Dropdown({ options, value, onChange, placeholder = 'Выберите...', className }: Props) {
  ...
  const handleListKeyDown = (e: KeyboardEvent<HTMLDivElement>) => {
    ...
    else if (e.key === 'ArrowDown') { e.preventDefault(); buttons[Math.min(idx + 1, buttons.length - 1)]?.focus() }
    else if (e.key === 'ArrowUp') { e.preventDefault(); idx <= 0 ? close() : buttons[idx - 1]?.focus() }
  }
